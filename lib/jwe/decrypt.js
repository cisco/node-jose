    ...
    value: function(input, opts) {
      ...
      promise = promise.then(function(jwe) {
        ...
        handlerKeys.forEach(function(h) {
          var hv = extraHandlers[h];
          var p;
          if ("function" === typeof hv) {
            p = hv(jwe);
          } else if ("object" === typeof hv && "function" === typeof hv.prepare) {
            p = hv.prepare(jwe);
        } else if ("boolean" === typeof hv && hv !== jwe.header.hasOwnProperty(h)) {
              p = Promise.reject(new Error("bad crit header"))
          }
          if (p) {
