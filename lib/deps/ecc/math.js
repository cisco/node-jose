...
function pointFpGetX() {
  if(!this.zinv) {
    // The jsbn `modInverse` function sometimes produces negative results, which
    // would make `r` negative.  And the `reduce` function cannot handle negative
    // numbers (it causes an endless loop).  The extra `mod` here maps any negative
    // value `-x` to a positive value `p - x`.
    this.zinv = this.z.modInverse(this.curve.p).mod(this.curve.p);
  }
...
function pointFpGetY() {
  if(!this.zinv) {
    // Same comment about the extra `mod` here as for `pointFpGetX`
    this.zinv = this.z.modInverse(this.curve.p).mod(this.curve.p);
  }
