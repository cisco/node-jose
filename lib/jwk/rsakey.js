...
var JWKRsaCfg = {
  ...
  convertToDER: function(key, isPrivate) {
    var k = rsau.convertToForge(key, !isPrivate);
    if (!isPrivate) {
      return forge.asn1.toDer(forge.pki.publicKeyToAsn1(k)).native();
    }
    return forge.asn1.toDer(forge.pki.privateKeyToAsn1(k)).native();
  }
};
